...
const { match, isFunction, isPlainObject, isString } = utils_1;



const { registerCommonMetrics, updateCommonMetrics } = commons;
...
class MetricRegistry {
	...
	init() {
		if (this.opts.enabled) {
			...
			if (this.opts.reporter) {
				...
				this.reporter = _.compact(reporters$1).map(r => {
	...
	timer(name, labels, timestamp) {
		...
		return () => {
			...
			if (item) {
				if (item.type == constants.TYPE_HISTOGRAM)
					item.observe(duration, labels, timestamp);
				else if (item.type == constants.TYPE_GAUGE)
