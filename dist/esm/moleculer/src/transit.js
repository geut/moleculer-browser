...
class Transit {
	...
	sendDisconnectPacket() {
		return this.publish(new Packet(packets.PACKET_DISCONNECT)).catch(/* istanbul ignore next */ err => this.logger.debug("Unable to send DISCONNECT packet.", err));
	}
	...
	_sendRequest(ctx, resolve, reject) {
		...
		const packet = new Packet(packets.PACKET_REQUEST, ctx.nodeID, payload);
				...
				if (isStream) {
					...
					stream.on("data", (chunk) => {
						...
						for (const ch of chunks) {
							...
							this.publish(new Packet(packets.PACKET_REQUEST, ctx.nodeID, copy))
								.catch(publishCatch);
						}
						stream.resume();
						return;
					...
					stream.on("end", () => {
						...
						return this.publish(new Packet(packets.PACKET_REQUEST, ctx.nodeID, copy))
							.catch(publishCatch);
					});
	...
	sendEvent(ctx) {
		...
		return this.publish(new Packet(packets.PACKET_EVENT, ctx.endpoint ? ctx.nodeID : null, {
			id: ctx.id,
			event: ctx.eventName,
			data: ctx.params,
			groups,
		...
		if (data && data.readable === true && typeof data.on === "function" && typeof data.pipe === "function") {
			...
			stream.on("data", (chunk) => {
				...
				for (const ch of chunks) {
					...
					this.publish(new Packet(packets.PACKET_RESPONSE, nodeID, copy))
						.catch(publishCatch);
				}
				stream.resume();
				return;
			...
			stream.on("end", () => {
				...
				return this.publish(new Packet(packets.PACKET_RESPONSE, nodeID, copy))
					.catch(publishCatch);
			});
			...
			payload.data = null;
			return this.publish(new Packet(packets.PACKET_RESPONSE, nodeID, payload))
				.then(() => {
					if (payload.stream)
						stream.resume();
				})
	 ...
	 */
	discoverNodes() {
		return this.publish(new Packet(packets.PACKET_DISCOVER))
	 ...
	 */
	discoverNode(nodeID) {
		return this.publish(new Packet(packets.PACKET_DISCOVER, nodeID))
	...
	sendNodeInfo(info, nodeID) {
		...
		return p.then(() => this.publish(new Packet(packets.PACKET_INFO, nodeID, {
	 ...
	 */
	sendPing(nodeID, id) {
		return this.publish(new Packet(packets.PACKET_PING, nodeID, { time: Date.now(), id: id || this.broker.generateUid() }))
	 ...
	 */
	sendPong(payload) {
		return this.publish(new Packet(packets.PACKET_PONG, payload.sender, {
			time: payload.time,
			id: payload.id,
			arrived: Date.now()
	...
	sendHeartbeat(localNode) {
		return this.publish(new Packet(packets.PACKET_HEARTBEAT, null, {
			cpu: localNode.cpu
